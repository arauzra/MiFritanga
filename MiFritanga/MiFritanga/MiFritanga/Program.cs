...
void EjecutarDia()
{
    ...
    // Asumiremos un salario de 350 al dia por cada empleado
    double gastosDia = empleados * 350.0;
...
void AbrirTienda()
{
    ...
    while (enTienda)
    {
        ...
        Console.WriteLine("╚══════════════════════════════════════╝");
        // Mostramos la plata disponible usando la variable global 'dinero'
        ...
        switch (opcion)
        {
            ...
            case "3":
                ...
                break;
            case "4":
                enTienda = false; // Rompe el ciclo y regresa al bucle principal
                break;
            default:
...
int GenerarClientes()
{
    // Calculamos la base según la capacidad del negocio
    int baseClientes = mesas * empleados;

    // Calculamos el bonus basado en la reputación
    int bonus = reputacion / 20;

    // Generamos un factor aleatorio entre -2 y 2 (el Next(min, max) excluye el máximo)
    int aleatorio = rng.Next(-2, 3);

    // Sumamos todos los factores
    int totalClientes = baseClientes + bonus + aleatorio;

    // Nos aseguramos de que no devuelva un número negativo
    if (totalClientes < 0)
    {
        totalClientes = 0;
    }
...
{
    // Armamos una línea de texto separada por comas (formato CSV)
    // Orden: Día, Ganancias, Gastos, Clientes Atendidos, Clientes Perdidos, Reputación
    string linea = $"{numeroDia},{ganancias:F2},{gastos:F2},{clientesAtendidos},{clientesPerdidos},{reputacion}";

    try
    {
        // Esto crea el archivo si no existe, o le agrega una línea nueva al final si ya existe
        File.AppendAllText(rutaHistorial, linea + "\n");
    }
    catch (Exception ex)
    {
        // Por si acaso hay algún error guardando el archivo (ej: falta de permisos)
        Console.WriteLine($"[!] Error al guardar el historial del día: {ex.Message}");
    }
}
